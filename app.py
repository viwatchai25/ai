...
try:
    # ดึง API Key จาก Secrets (ตรวจสอบว่าคุณเปลี่ยน Key ใหม่ที่ยังไม่โดนระงับแล้ว)
    ...
    st.error("⚠️ ไม่พบ API Key กรุณาตั้งค่า GEMINI_API_KEY ใน Streamlit Secrets")
    st.stop()
        ...
        with st.chat_message("assistant"):
            with st.spinner("Digital CMRU AI กำลังประมวลผล..."):
                try:
                    ...
                    # เรียกใช้ Gemini 1.5 Flash (โควตาสูงกว่า 2.0/2.5 สำหรับสายฟรี)
                    ...
                    # ตรวจสอบ Error 429 (โควตาเต็ม)
                    if "429" in str(e):
                        st.error(
                            "⚠️ ขออภัยครับ โควตาการใช้งานชั่วคราวเต็มแล้ว (Rate Limit) กรุณารอสัก 1-2 นาทีแล้วลองใหม่ครับ")
                    else:
                        st.error(f"เกิดข้อผิดพลาดในการประมวลผล: {e}")
    else:
        st.warning("ระบบยังไม่มีฐานข้อมูล กรุณาติดต่อ Admin เพื่ออัปโหลดไฟล์ data.pdf")